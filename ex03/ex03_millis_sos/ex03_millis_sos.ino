...
void loop() {
  ...
  switch (state) {
    ...
    case PAUSE:
      ...
      // 判断：当前时间 - 上次记录时间 >= 延时时间 → 时间到了，该切换状态了
if (now - previousMillis >= pauseDuration) {
  previousMillis = now;  // 刷新时间，重新计时

  // flashCount 用来标记：现在做到 SOS 的哪一步了
  if (flashCount == 0) {
    flashCount = 1;       // 标记：第一段 S 做完了
    state = LONG_ON;      // 切换到：长亮（开始做 O）
  } 
  else if (flashCount == 1) {
    flashCount = 2;       // 标记：O 做完了
    state = SHORT_ON;     // 切换到：短亮（开始做第二段 S）
  } 
  else {
    // 第二段 S 也做完了 → 一整套 SOS 完成！
    flashCount = 0;       // 全部重置
    state = SHORT_ON;     // 重新开始第一遍 S
  }
}
  }
}
